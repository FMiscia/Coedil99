...
public class StandardOttimizzatoreStrategy extends AOttimizzatoreStrategy {
	...
	public DocumentoOttimizzazione elaboraOttimizzazione(Distinta distinta) {
		...
		ArrayList<MItem> itm = new ArrayList<MItem>(MMagazzino.getInstance().getItems().keySet());
		//Item da cancellare
		//ArrayList<Item> rem = new ArrayList<Item>();

		
		float offset;
		Iterator<RigaLavoro> it = distinta.lavori.getIterator();
		while (it.hasNext()) {
		...
		for (int i = 0; i < itm.size(); i++) {
			for (int j = 0; j < distitems.size(); j++) {
				if ((equal(itm.get(i).getPersistentModel().getGeometria().getBase(), distitems.get(j).getBase(), itm.get(i).getPersistentModel().getGeometria().getAltezza(), distitems.get(j).getAltezza()))
						&& ((itm.get(i).getPersistentModel().getGeometria().getLunghezza() - distitems.get(j).getLunghezza()) <= 100)) {
		...
		for (int i = 0; i < itm.size(); i++) {
			for (int j = 0; j < distitems.size(); j++) {
				if ((equal(itm.get(i).getPersistentModel().getGeometria().getBase(), distitems
						.get(j).getBase(), itm.get(i).getPersistentModel().getGeometria()
						.getAltezza(), distitems.get(j).getAltezza()))
						&& ((offset = itm.get(i).getPersistentModel().getGeometria().getLunghezza()
		...
		for (int i = 0; i < ddoitem.size(); ++i) {
			o.items.add(ddoitem.get(i).getPersistentModel());
		}
