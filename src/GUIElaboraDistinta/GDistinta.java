...
public class GDistinta {
	...
	public GDistinta(final int index) {
		...
		final GestisciCommessaHandler gch = GestisciCommessaHandler
				.getInstance();

		f = new JFrame("Distinta "
				+ gch.getCommessaByIndex(index).getCodiceInterno());
		...
		String[] cl6 = { "Base", "Altezza", "Lunghezza", "Numero", "Capitello",
				"TipoCap", "Note" };
		...
		save.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent arg0) {
				ODistinta d = new ODistinta(new Distinta());
				for (int i = 0; i < t.getTable().getRowCount(); i++) {

					d.addRigaLavoro(t.getRow(i));
				}
				gch.associaDistinta(d.getDistinta(),
						gch.getCommessaByIndex(index).getId());
		...
		if (gch.getCommessaByIndex(index).getDistinta() == null
				|| (gch.getCommessaByIndex(index).getDistinta() != null && !gch
						.getCommessaByIndex(index).getOdistinta().hasDdo()))
		...
		this.ottimizza.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent arg0) {
				gch.getCommessaByIndex(index).getOdistinta().creaDDO();
